...
@Slf4j
@Repository
public class UndervisningsgruppeRepository implements Handler {
    ...
    @Getter
    private Collection<UndervisningsgruppeResource> repository = new ConcurrentLinkedQueue<>();
    ...
    @PostConstruct
    public void init() {
        repository.addAll(fakeData.getUndervisningsgrupper());
        log.info("Repository contains {} items.", repository.size());
    }

    @Override
    public Set<String> actions() {
        return ImmutableSet.of(TimeplanActions.GET_ALL_UNDERVISNINGSGRUPPE.name());
    }
    ...
    public void accept(Event<FintLinks> response) {
        ...
        if (TimeplanActions.valueOf(response.getAction()) == TimeplanActions.GET_ALL_UNDERVISNINGSGRUPPE) {
